...
class ensemble_DA:
    ...
    def get_ensemble_block(self, msk_cor):
        ...
        if self.test == 1:
            print("* ENDJ - Working on block {0}".format(msk_cor))
        for v in msk_cor:
...
class EnKF_MC_obs(ensemble_DA):
    def prepare_background(self):
        ...
        for msk_cor, pre_info in zip(mask_cor, gr.lpr):
            ...
            Binv_sqrt_block = compute_modified_Cholesky_decomposition(
                DX_block, pre_info, thr=0.15
            )

            self.XB_map.append(
                {"XB_b": XB_block, "xb_b": xb_block, "Binv_s_b": Binv_sqrt_block}
            )
    ...
    def perform_assimilation(self, ob):
        self.XA_map = []
        for XB_info, Ys_block, R_info, H_block in zip(
            self.XB_map, self.Ys, ob.obs_R_sparse, ob.obs_H_sparse
        ):
            XB_block = XB_info["XB_b"]
            R_block = R_info["R"]
            Binv_sqrt_block = XB_info["Binv_s_b"]
            XA_block = self.perform_assimilation_block(
                XB_block, Binv_sqrt_block, H_block, R_block, Ys_block
            )
            XA_block = self.covariance_inflation(XA_block)
            self.XA_map.append(XA_block)
        self.map_vector_states()
        # Update ensemble folders
...
class LETKF(ensemble_DA):
    ...
    def perform_assimilation(self, ob):
        self.XA_map = []
        for XB_info, y_block, R_info, H_block, lobs_block in zip(
            self.XB_map, self.y, ob.obs_R_sparse, ob.obs_H_sparse, self.nm.gs.lbo_obs
        ):
            XB_block = XB_info["XB_b"]
            xb_block = XB_info["xb_b"]
    ...
    def perform_assimilation_block(self, XB, xb, DX, H, Ri, y, lbo_info, lobs):
        ...
        lbo, nlbo = lbo_info
        # local boxes information (indexes, number of components in all boxes)
        ...
        for i in range(0, n):
            ...
            H_ind = np.array(lobs[i]).astype("int32")
            # local observed components
            # print(f'H_ind {H_ind}');
            m_i = H_ind.size
            if m_i > 0:
                n_i = xb_i.size
                ...
                Ri_i = np.diag(Ri_space[lbo_i[H_ind], lbo_i[H_ind]]).reshape((m_i, m_i))
                # local data error covariance matrix
                XA_i = self.perform_assimilation_local_box(
                    XB_i, xb_i, DX_i, H_i, Ri_i, y_i
                )
...
class LEnKF(ensemble_DA):
    ...
    def perform_assimilation(self, ob):
        self.XA_map = []
        for XB_info, Ys_block, R_info, H_block, lobs_block in zip(
            self.XB_map, self.Ys, ob.obs_R_sparse, ob.obs_H_sparse, self.nm.gs.lbo_obs
        ):
            XB_block = XB_info["XB_b"]
            lbo_block = XB_info["lbo_b"]
            Ri_block = R_info["Ri"]
            XA_block = self.perform_assimilation_block(
                XB_block, H_block, Ri_block, Ys_block, lbo_block, lobs_block
            )
            self.XA_map.append(XA_block)
        self.map_vector_states()
        # Update ensemble folders
...
class sequential_method:
    ...
    def get_instance(self, nm, infla, Nens):
        if self.method_name == "EnKF_MC_obs":
            return EnKF_MC_obs(nm, infla, Nens)
        if self.method_name == "LETKF":
            return LETKF(nm, infla, Nens)
        if self.method_name == "LEnKF":
            return LEnKF(nm, infla, Nens)
